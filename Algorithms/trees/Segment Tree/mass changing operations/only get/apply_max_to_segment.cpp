...
#define endl '\n'
template <typename T>   
struct ApplyMaxToSegmentTree
{
    ApplyMaxToSegmentTree(size_t size)
    : n(size)
    {
        tree.assign(n << 2, Node());
    }
    ApplyMaxToSegmentTree(const vector<T>& vec)
    : n(vec.size())
    {
        tree.assign(n << 2, Node());
        build(vec);
...
private:
    ...
    T get(int pos, int idx, int l, int r)
    {
        if(l + 1 == r)
        {
            return tree[idx].mx;
        ...
        if(pos < mid) return max(tree[idx].mx, get(pos, idx * 2 + 1, l, mid));
        else return max(tree[idx].mx, get(pos, idx * 2 + 2, mid, r));
    }

...
#define endl '\n'
template <typename T>   
struct AddValueToSegmentTree
{
    AddValueToSegmentTree(size_t size)
    : n(size)
    {
        tree.assign(n << 2, Node());
    }
    AddValueToSegmentTree(const vector<T>& vec)
    : n(vec.size())
    {
        tree.assign(n << 2, Node());
        build(vec);
...
private:
    ...
    T get(int pos, int idx, int l, int r)
    {
        ...
        if(pos < mid) return tree[idx].dif + get(pos, idx * 2 + 1, l, mid);
        else return tree[idx].dif + get(pos, idx * 2 + 2, mid, r);
    }

...
template<typename T> 
struct MinAssignTree
{
    MinAssignTree(size_t size, T def = (T) 0)
    : n(size)
    , def(def)
    {
        tree.assign(n * 4, {.clear = true, .val = 0, .mn = def});
    }
    MinAssignTree(const vector<T>& vec)
    : n(vec.size())
    {
...
private:
    ...
    struct Node
    {
        bool clear = true;
        T val = 0;
        T mn = (sizeof(T) == 32) ? INT_MAX / 2 : LLONG_MAX / 2;
    };
    ...
    void push(int idx, int L, int R)
    {
        ...
        tree[l].val = tree[r].val = tree[l].mn = tree[r].mn = tree[idx].mn = tree[idx].val;
    ...
    void update(int idx, int l, int r, int curL, int curR, T val)
    {
        ...
        update(idx * 2 + 1, l, min(r, mid), curL, mid, val);
        update(idx * 2 + 2, max(l, mid), r, mid, curR, val);

        tree[idx].mn = (tree[idx].clear) ? min(tree[idx * 2 + 1].mn, tree[idx * 2 + 2].mn) : tree[idx].val;
    }
    T getMin(int idx, int l, int r, int curL, int curR)
    {
        if(l >= r) return (sizeof(T) == 32) ? INT_MAX / 5 : LLONG_MAX / 5;
        
        if(!tree[idx].clear) return tree[idx].mn;

        if(l == curL && r == curR) return tree[idx].mn;

        int mid = (curL + curR) / 2;

        return min(getMin(idx * 2 + 1, l, min(r, mid), curL, mid), getMin(idx * 2 + 2, max(l, mid), r, mid, curR));
    }

...
void dfs2(const vector<vector<int>>& g, const vector<int>& heavyEdge, vector<int>& leader, vector<int>& parent, vector<int>& tin, vector<int>& tout, int& time, int cur, int prev)
{
    tin[cur] = time++;
    ...
    if(heavyEdge[cur])
    {
        ...
        dfs2(g, heavyEdge, leader, parent, tin, tout, time, heavyEdge[cur], cur);
    }
    ...
    for(auto&& to : g[cur])
    {
        ...
        parent[to] = cur;
        leader[to] = to;
        dfs2(g, heavyEdge, leader, parent, tin, tout, time, to, cur);
    }
    tout[cur] = time;
}
class Tree
...
void solve()
{
    ...
    vector<int> depth(n + 1), heavyEdge(n + 1), leader(n + 1), parent(n + 1), tin(n + 1), tout(n + 1);
    depth[root] = 1;
    dfs1(g, heavyEdge, depth, root);
    leader[root] = root;
    int time = 0;
    dfs2(g, heavyEdge, leader, parent, tin, tout, time, root, -1);
    Tree ST(n);
    ...
    auto set = [&](int a, int b, int val)
    {
        ...
        while(true)
        {
            ...
            {
                ST.update(tin[lca] + 1, tin[a] + 1, val);
                break;
            }

            ST.update( tin[leader[a]], tin[a] + 1, val);
            a = parent[leader[a]];
        }
        while(true)
        {
            ...
            {
                ST.update(tin[lca], tin[b] + 1, val);
                break;
            }

            ST.update(tin[leader[b]], tin[b] + 1, val);
            b = parent[leader[b]];
        }
    };
    int q;
    ...
    for(int i = 0; i < q; ++i)
    {
        ...
        {
//            assert(ST.getInPoint(v) == ST.getSum(tin[v], tin[v] + 1));
            cout << ST.getInPoint(tin[v]) << endl;
        }
    }
}
int32_t main()
